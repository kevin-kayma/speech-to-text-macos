export 'apptheme.dart';
export 'constants.dart';
export 'globals.dart';
export 'strings.dart';
export 'utils.dart';

//Packages
export 'package:hooks_riverpod/hooks_riverpod.dart';
export 'package:responsive_sizer/responsive_sizer.dart';
export 'package:flutter_hooks/flutter_hooks.dart';
export 'package:transcribe/models/initalertdata.dart';
export 'package:url_launcher/url_launcher.dart';
export 'package:hive/hive.dart';
export 'package:hive_flutter/hive_flutter.dart';
export 'package:share_plus/share_plus.dart';
export 'package:in_app_review/in_app_review.dart';

export 'package:path_provider/path_provider.dart';
// export 'package:firebase_core/firebase_core.dart';
// export 'package:firebase_crashlytics/firebase_crashlytics.dart';
// export 'package:firebase_analytics/firebase_analytics.dart'; //TODO:
export 'package:internet_connection_checker/internet_connection_checker.dart';
// export 'package:google_mobile_ads/google_mobile_ads.dart';
export 'package:introduction_screen/introduction_screen.dart';
// export 'package:easy_localization/easy_localization.dart';
export 'package:speech_to_text/speech_to_text.dart';
export 'package:lottie/lottie.dart';
export 'package:record/record.dart';
export 'package:open_store/open_store.dart';
// export 'package:flutter_tts/flutter_tts.dart';
export 'package:package_info_plus/package_info_plus.dart';
export 'package:flutter_dotenv/flutter_dotenv.dart';
// export 'package:font_awesome_flutter/font_awesome_flutter.dart';
// export 'package:selectable_autolink_text/selectable_autolink_text.dart';
export 'package:quick_actions/quick_actions.dart';
// export 'package:tiktoken/tiktoken.dart';

// export 'package:singular_flutter_sdk/singular.dart';
export 'package:deepgram_speech_to_text/deepgram_speech_to_text.dart';
// export 'package:flutter_markdown/flutter_markdown.dart';
export 'package:hugeicons/hugeicons.dart';
// export 'package:image_picker/image_picker.dart';
export 'package:slack_notifier/slack_notifier.dart';
// export 'package:fluttertoast/fluttertoast.dart';

export 'dart:convert';
export 'package:flutter/material.dart';
export 'package:transcribe/config/store_config.dart';
export 'package:selectable_autolink_text/selectable_autolink_text.dart';
export 'package:macos_ui/macos_ui.dart';
export 'package:bmprogresshud/bmprogresshud.dart';
