...
class ApiService {
  ...
  static Future<Responsemodel?> speechToTextDG(String filePath, String languageCode, BuildContext context) async {
    ...
    try {
      ...
      if (jsonResponse.containsKey('err_code')) {
        Utils.sendErrorToSlack('Deepgram Error: ${jsonResponse['err_msg']}', statusCode: jsonResponse['err_code']);
        return null;
      }

      // Convert the response to ResTranscriptionResponse
      ResTranscriptionResponse transcriptionResponse = ResTranscriptionResponse.fromJson(jsonResponse);

      // Map the ResTranscriptionResponse to Responsemodel
      Responsemodel responseModel = mapResTranscriptionResponseToResponseModel(transcriptionResponse);
  ...
  static Responsemodel mapResTranscriptionResponseToResponseModel(ResTranscriptionResponse transcriptionResponse) {
    ...
    final paragraphs = Paragraphs(
      transcript: transcriptionResponse.results.channels
          ...
          .map((alt) => utf8.decode(alt.transcript.codeUnits)) // Extract transcript from ResAlternative
      ...
      paragraphs: transcriptionResponse.results.channels
          ...
          .expand((alt) => alt.paragraphs.paragraphs) // Extract all ResParagraphs
          .map((resParagraph) {
        ...
        return Paragraph(
          sentences: resParagraph.sentences.map((resSentence) {
            return Sentence(
              ...
              confidence: 0.0, // Since 'confidence' isn't available, default to 0.0
            );
          }).toList(),
